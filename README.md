...
from betweak_django_cities.models import BaseCountry
...
```bash
python manage.py migrate betweak_django_cities
```
...
```python
CITIES_SLUGIFY_FUNCTION = 'betweak_django_cities.util.default_slugify'
```
...
```python
# Import betweak_django_cities without region (default False)
...
CITIES_PLUGINS = [
    ...
    'betweak_django_cities.plugin.postal_code_ca.Plugin',
    # Reduce memory usage when importing large datasets (e.g. "allCountries.zip")
    'betweak_django_cities.plugin.reset_queries.Plugin',
]
```
...
```python
# This setting may be specified if you use 'betweak_django_cities.plugin.reset_queries.Plugin'
...
```bash
python manage.py betweak_django_cities --import=all
```
...
```bash
python manage.py betweak_django_cities --import=country
```

```bash
python manage.py betweak_django_cities --import=city
```
...
class CompleteSkeletonPlugin(object):
    """
    Skeleton plugin for django-betweak_django_cities that has hooks for all object types, and
...
```python
from betweak_django_cities.conf import HookException

class DorothyPlugin(object):
    """
    This plugin skips importing betweak_django_cities that are not in Kansas, USA.
    
    There's no place like home.
    """
    def city_pre(self, parser, import_dict):
        if import_dict['cc2'] == 'US' and import_dict['admin1Code'] != 'KS':
            raise HookException("Ignoring betweak_django_cities not in Kansas, USA")  # Raising a HookException skips importing the item
        else:
...
# 5 Nearest betweak_django_cities to London
>>> london = City.objects.filter(country__name='United Kingdom').get(name='London')
>>> nearest = City.objects.distance(london.location).exclude(id=london.id).order_by('distance')[:5]

# All betweak_django_cities in a state or county
...
```bash
python manage.py migrate betweak_django_cities 0001 --fake
```
